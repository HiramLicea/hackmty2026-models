...
async def require_inference_api_key(
    ...
    """Require the shared inference credential without exposing it to logs."""
    configured_secret = settings.inference_api_key
    if configured_secret is None or not configured_secret.get_secret_value():
        raise ServiceNotConfiguredError("INFERENCE_API_KEY is required for protected operations")
